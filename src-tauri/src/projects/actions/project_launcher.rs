...
pub async fn launch_project_with_engine(
    ...
    info!("Launching project with Unreal Engine: {}", project_path.display());
    log(&app_handle, ErrorLevel::Info, &format!("Launching project: {}", project_path.display()));
    ...
    match result {
        Ok(_) => {
            log(&app_handle, ErrorLevel::Info, "Project launched successfully");
            Ok(())
        }
        Err(e) => {
...
pub async fn launch_project_with_ide(
    ...
    info!("Launching project with IDE: {} -> {}", ide_path.display(), file_to_open.display());
    log(&app_handle, ErrorLevel::Info, &format!(
        "Launching project with IDE: {}", 
        file_to_open.display()
    ));
    
    let result = Command::new(&ide_path)
        .arg(&file_to_open)
        .spawn();
    
    match result {
        Ok(_) => {
            log(&app_handle, ErrorLevel::Info, "Project launched with IDE successfully");
            Ok(())
        }
        Err(e) => {
