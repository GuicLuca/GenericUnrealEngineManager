...
use crate::misc::payloads::{CompressionRequest, CompressionResult};
use crate::misc::prelude::{format_size, log};
use crate::misc::progress::TaskProgress;
use crate::projects::actions::behavior;
use crate::projects::actions::project_cleaner::clean_project;
...
pub async fn compress_project(
    ...
    log(
        &app_handle,
        ErrorLevel::Info,
        &format!("Starting compression process for project: {}", project_name),
    );
    ...
    if request.clean_before_compress {
        if let Some(cleaning_selection) = request.cleaning_selection {
            progress.update(
                0.1,
                Some("Cleaning project before compression...".to_string()),
            );
            log(
                &app_handle,
                ErrorLevel::Info,
                "Cleaning project before compression...",
            );
            ...
            .await
            {
                Ok(_) => {
                    log(
                        &app_handle,
                        ErrorLevel::Info,
                        "Project cleaned successfully before compression",
                    );
                }
                Err(e) => {
    ...
    log(
        &app_handle,
        ErrorLevel::Info,
        &format!("Compressing to: {}", output_path.display()),
    );
    ...
    progress.update(
        ...
        Some(format!(
            "Creating {} archive...",
            get_algorithm_name(&request.compression_algorithm)
        )),
    );
...
fn generate_filename(
    ...
    replacements.insert(
        "Algorithm".to_string(),
        get_algorithm_name(algorithm).to_string(),
    );
    replacements.insert(
    ...
    replacements.insert("User".to_string(), behavior::get_system_username());
    replacements.insert("Computer".to_string(), behavior::get_system_hostname());
