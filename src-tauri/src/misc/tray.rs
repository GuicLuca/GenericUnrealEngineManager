...
pub fn update_tray_menu(app: &AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hide_item = MenuItem::with_id(app, "hide", "Hide Project Manager", true, None::<&str>)?;
    if let Some(window) = app.get_webview_window("main") {
        if !window.is_visible().unwrap() {
            hide_item.set_text("Show Project Manager")?;
        } else {
            hide_item.set_text("Hide Project Manager")?;
        }
    }
